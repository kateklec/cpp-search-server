...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const string& word : stop_words_) {
            if (!IsValidWord(word))
                throw invalid_argument("недопустимые символы в стоп-слове");
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (documents_.count(document_id) > 0)
            throw invalid_argument("документ ранее добавлен");

        vector<string> words;
        if (!SplitIntoWordsNoStop(document, words)) {
            throw invalid_argument("недопустимые символы в тексте добавляемого документа");
        }
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        ...
        if (!ParseQuery(raw_query, query)) {
            throw invalid_argument("неправильный запрос");
        }
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        return pair{ matched_words, documents_.at(document_id).status };
...
private:
    ...
    optional<QueryWord>ParseQueryWord(string text, QueryWord& result) const {
